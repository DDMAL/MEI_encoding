...
def build_mei(pairs, staves, classifier, page):
    ...
    surface.setAttribute('ulx', surface_bb['ulx'])
    surface.setAttribute('uly', surface_bb['uly'])
    surface.setAttribute('lrx', surface_bb['lrx'])
    surface.setAttribute('lry', surface_bb['lry'])
...
def merge_nearby_neume_components(meiDoc, width_multiplier=1):
    ...
    for c in surface.getChildren():
        ...
        surf_dict[c.id] = c
    med_neume_width = np.mean(neume_widths) * width_multiplier
